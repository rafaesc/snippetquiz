...
class GroqClient:
    ...
    def generate_completion(
        self, messages: List[dict], max_tokens: int = 4096, stream: bool = False
    ) -> Optional[str]:
        """
    ...
    def generate_topics(
        ...
        if response:
            ...
            print(f"Failed raw topics response after retry: {response}")
    ...
    def generate_quiz_questions(
        ...
        if response:
            ...
            try:
                cleaned_response = self.extract_block_between_braces(response)
                ...
                if retry_response:
                    try:
                        cleaned_retry_response = self.extract_block_between_braces(
                            retry_response
                        )
