...
class KafkaTopicConsumer:
    ...
    def _send_save_event_to_kafka(self, user_id, content_id, generated_topics):
        ...
        try:
            ...
            future = self.producer.send(
                "content-entry-events", key=key, value=save_event
            )
            ...
            print(
                f"✅ Successfully sent SAVE event to Kafka topic 'content-entry-events'"
            )
            print(
                ...
                if message_batch:
                    for topic_partition, messages in message_batch.items():
                        ...
                        for message in messages:
                            print(
                                f"📨 Message offset: {message.offset}, key: {message.key}, topic: {message.topic}"
                            )
                            try:
                                # Try to put message in queue with timeout
                                self.message_queue.put(message, timeout=5)
                                print(
                                    f"📨 Queued message from {message.topic} (queue size: {self.message_queue.qsize()})"
                                )
                            except queue.Full:
                                print(
                                    f"⚠️ Message queue full, applying backpressure. Dropping message from {message.topic}"
                                )
        ...
        while self.worker_running:
            try:
                ...
                try:
                    ...
                    print(
                        f"✅ Processed message from {message.topic} (queue size: {self.message_queue.qsize()})"
                    )

                except Exception as processing_error:
                    print(
                        f"❌ Error processing message from {message.topic}: {processing_error}"
                    )
    ...
    def start_consumer(self):
        ...
        while not self.running:
            try:
                ...
                print(
                    f"✅ Kafka consumer started with lightweight poller pattern on {kafka_brokers}"
                )

                # Start the poller and worker threads
                self.poller_thread = threading.Thread(
                    target=self._lightweight_poller, daemon=True
                )
                ...
                print(
                    f"⚠️ Unexpected error in Kafka consumer: {e}, retrying in {retry_delay}s ..."
                )
                time.sleep(retry_delay)
    ...
    def handle_generate_topics_message(self, message_data):
        ...
        try:
            ...
            content_id = message_data.get("contentId", None)
            ...
            if content_entry and content_entry.get("topics"):
                ...
                if topics_list and len(topics_list) > 0:
                    print(
                        f"Content entry {content_id} already has topics: {topics_list}. Skipping topic generation."
                    )
                    return
            ...
            existing_topics = (
                [
                    topic.strip()
                    for topic in existing_topics_str.split(",")
                    if topic.strip()
                ]
    ...
    def handle_create_quiz_message(self, message_data):
        ...
        try:
            ...
            user_id = message_data.get("userId", "")
            quiz_id = message_data.get("quizId", None)
            instructions = message_data.get("instructions", "")
            content_entries = message_data.get("contentEntries", [])
            total_content_entries_skipped = message_data.get(
                "totalContentEntriesSkipped", 0
            )
            bank_id = message_data.get("bankId", None)

            print(
                f"📋 Received CREATE QUIZ event Quiz ID: {quiz_id}, User ID: {user_id}, Bank ID: {bank_id}, Content Entries Count: {len(content_entries)}"
            )
            ...
            if quiz_response:
                ...
                if quiz_status != "PREPARE":
                    print(
                        f"⏭️ Quiz {quiz_id} status is {quiz_status}, skipping processing (already completed or in progress)"
                    )
                    ...
                    if hasattr(self, "consumer") and self.consumer:
                        try:
                            ...
                            print(
                                f"✅ Message committed for quiz {quiz_id} (idempotency)"
                            )
                        except Exception as commit_error:
                            print(
                                f"⚠️ Commit failed for idempotency check: {commit_error}"
                            )
                    return
            else:
                print(
                    f"⚠️ Could not fetch quiz status for {quiz_id}, proceeding with processing"
                )
            ...
            print(
                f"📊 Total chunks to process: {total_chunks} (chunk size: {chunk_size}) Quiz ID: {quiz_id}, User ID: {user_id}, Bank ID: {bank_id}"
            )
            ...
            for entry_index, entry in enumerate(content_entries):
                ...
                if content:
                    ...
                    print(
                        f"  📄 Entry {entry_index + 1} (ID: {entry_id}): '{page_title}' - {len(content)} chars -> {entry_chunks} chunks"
                    )
                    ...
                    for chunk_index in range(entry_chunks):
                        ...
                        try:
                            ...
                            for q_data in result.get("questions", []):
                                ...
                                for opt_data in q_data.get("options", []):
                                    option = {
                                        ...
                                        "optionExplanation": opt_data.get(
                                            "explanation", ""
                                        ),
                            ...
                            print(
                                f"Generated {len(chunk_questions)} questions from chunk {chunk_index + 1}"
                            )

                        except Exception as e:
                            print(
                                f"Error generating questions for chunk {chunk_index + 1}: {e}"
                            )
                        ...
                        # Send quiz generation event to Kafka
                        self._send_quiz_generation_event_to_kafka(
                            quiz_generation_payload
                        )
                        ...
                        print(
                            f"📈 Total questions generated so far: {total_questions_generated}"
                        )
                        time.sleep(20)  # 20 second delay between chunks
                else:
                    print(
                        f"📄 Entry {entry_index + 1} (ID: {entry_id}): '{page_title}' - No content"
                    )
    ...
    def _send_quiz_generation_event_to_kafka(self, payload):
        ...
        try:
            ...
            future = self.producer.send("quiz-generation", key=key, value=payload)

            # Wait for the message to be sent (with timeout)
            record_metadata = future.get(timeout=10)

            print(
                f"✅ Successfully Topic: {record_metadata.topic}, Partition: {record_metadata.partition}, Offset: {record_metadata.offset}"
            )
            print(
                f"✅ Successfully sent quiz generation, Key: {key}, Quiz ID: {payload['quizId']}, Chunk: {payload['currentChunkIndex'] + 1}/{payload['totalChunks']}"
            )
