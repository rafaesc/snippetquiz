...
export class ContentEntryController {
  ...
  async create(
    ...
    try {
      // Validation
      if (!createContentEntryDto.type || !['full_html', 'selected_text', 'video_transcript'].includes(createContentEntryDto.type)) {
        throw new HttpException('Type must be either "full_html", "selected_text" or "video_transcript"', HttpStatus.BAD_REQUEST);
      }
      ...
      if (createContentEntryDto.type === 'full_html' && !createContentEntryDto.sourceUrl) {
        throw new HttpException('Source URL is required when type is "full_html"', HttpStatus.BAD_REQUEST);
      }

      if (createContentEntryDto.type === 'selected_text' && !createContentEntryDto.content) {
        throw new HttpException('Content is required when type is "selected_text"', HttpStatus.BAD_REQUEST);
      }

      if (createContentEntryDto.type === 'video_transcript' && !createContentEntryDto.sourceUrl) {
        throw new HttpException('Source URL is required when type is "video_transcript"', HttpStatus.BAD_REQUEST);
      }
