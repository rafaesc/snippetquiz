...
@Injectable()
export class CoreServiceProxyService {
  private readonly logger = new Logger(CoreServiceProxyService.name);
  private readonly baseUrl: string;

  constructor(private readonly httpService: HttpService) {
    this.baseUrl = `http://${envs.coreServiceHost}:${envs.coreServicePort}/api`;
  }
  ...
  async proxyRequest<T = any>(options: ProxyRequestOptions): Promise<T> {
    ...
    try {
      ...
      throw new HttpException(
        'Failed to communicate with core service',
