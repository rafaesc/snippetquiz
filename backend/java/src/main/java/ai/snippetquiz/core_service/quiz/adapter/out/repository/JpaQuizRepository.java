...
public interface JpaQuizRepository extends JpaRepository<QuizEntity, UUID> {
    ...
    Optional<QuizEntity> findByIdAndUserIdWithTopics(@Param("id") UUID id, @Param("userId") UUID userId);
    
    @Query("SELECT q FROM QuizEntity q LEFT JOIN FETCH q.quizQuestions WHERE q.id = :id AND q.userId = :userId")
    Optional<QuizEntity> findByIdAndUserIdWithQuestions(@Param("id") UUID id, @Param("userId") UUID userId);
    
    Optional<QuizEntity> findByIdAndUserId(UUID id, UUID userId);
}